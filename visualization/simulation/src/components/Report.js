...
import { TRIP_COLOR_PALETTE } from './Trip';
...
export default function Report(props){
    ...
    const [chartColors, setChartColors] = useState([
        `rgba(${TRIP_COLOR_PALETTE.dispatched.join(',')}, 0.8)`,  // 기본: Dispatch
        `rgba(${TRIP_COLOR_PALETTE.occupied.join(',')}, 0.8)`,  // 기본: Operate
        ...
        useEffect(() => {
            ...
            const operatingBar = document.querySelector('.operating-bar');
            const idleCircle = document.querySelector('.circle');
        
            if (dispatchBar)
              dispatchBar.style.backgroundColor = `rgb(${TRIP_COLOR_PALETTE.dispatched.join(',')})`;
            if (operatingBar)
              operatingBar.style.backgroundColor = `rgb(${TRIP_COLOR_PALETTE.occupied.join(',')})`;
            if (idleCircle)
              idleCircle.style.backgroundColor = `rgb(${TRIP_COLOR_PALETTE.idle.join(',')})`;
          }, []);
